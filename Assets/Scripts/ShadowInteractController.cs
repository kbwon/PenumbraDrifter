...
public class ShadowInteractController : MonoBehaviour
{
    ...
    [Header("Moving Shadow Host")]
    public float movingHostGraceSeconds = 0.12f;
    ...
    Transform currentMovingShadowHost;
    Collider currentMovingShadowHostCollider;
    Vector3 currentMovingShadowHostLastPosition;
    float movingHostGraceTimer;
    bool movingHostInitialized;
    ...
    public bool HasMovingShadowHost => currentMovingShadowHost != null;
    public Transform CurrentMovingShadowHost => currentMovingShadowHost;
    ...
    {
        return TryGetSurfacePointAtWorldPos(transform.position, out point, out normal, out _);
    }

    bool TryGetSurfacePointAtWorldPos(Vector3 worldPos, out Vector3 point, out Vector3 normal, out Collider surfaceCollider)
    {
        ...
        if (Physics.Raycast(originDown, Vector3.down, out RaycastHit hitDown, 10f, surfaceMask, QueryTriggerInteraction.Ignore))
        {
            ...
            surfaceCollider = hitDown.collider;
            return true;
        }
    ...
    {
        if (TryGetSurfacePointAtWorldPos(worldPos, out Vector3 point, out Vector3 normal, out _))
            return IsShadowAtPoint(point, normal);
    ...
    public bool IsShadowSafeAtWorldPos(Vector3 worldPos, float margin)
    {
        ...
        return IsShadowSafeAtPoint(point, normal, margin);
    }
    ...
    public bool RefreshMovingShadowHost(Vector3 riderWorldPos, float margin)
    {
        if (!inShadowMode)
        {
            ClearMovingShadowHost();
        ...
        if (!TryGetSurfacePointAtWorldPos(riderWorldPos, out Vector3 point, out Vector3 normal, out _))
            return StepMovingHostGrace();

        Collider bestCollider = FindBestMovingShadowHostCollider(point, normal, margin);
        if (bestCollider == null)
            return StepMovingHostGrace();

        Transform newHost = ResolveShadowHostTransform(bestCollider);
        bool hostChanged = newHost != currentMovingShadowHost;

        currentMovingShadowHost = newHost;
        currentMovingShadowHostCollider = bestCollider;
        movingHostGraceTimer = movingHostGraceSeconds;

        if (!movingHostInitialized || hostChanged)
        {
            currentMovingShadowHostLastPosition = currentMovingShadowHost.position;
            movingHostInitialized = true;
        }

        return true;
    }

    bool StepMovingHostGrace()
    {
        if (currentMovingShadowHost != null && movingHostGraceTimer > 0f)
        {
            movingHostGraceTimer = Mathf.Max(0f, movingHostGraceTimer - Time.fixedDeltaTime);
            return true;
        }
    ...
    public Vector3 ConsumeMovingShadowHostDelta()
    {
        if (currentMovingShadowHost == null || !movingHostInitialized)
            return Vector3.zero;

        Vector3 now = currentMovingShadowHost.position;
        Vector3 delta = now - currentMovingShadowHostLastPosition;
        currentMovingShadowHostLastPosition = now;
        return delta;
    }

    public void ClearMovingShadowHost()
    {
        currentMovingShadowHost = null;
        currentMovingShadowHostCollider = null;
        currentMovingShadowHostLastPosition = Vector3.zero;
        movingHostGraceTimer = 0f;
        movingHostInitialized = false;
    }

    Collider FindBestMovingShadowHostCollider(Vector3 point, Vector3 normal, float margin)
    {
        normal = normal.normalized;
        BuildTangents(normal, out Vector3 t1, out Vector3 t2);

        float sampleMargin = Mathf.Max(0.02f, margin * movingHostSampleMarginFactor);
        ...
        for (int i = 0; i < offsets.Length; i++)
        {
            Vector3 testPoint = point + offsets[i];

            if (!TryGetShadowCasterAtPoint(testPoint, normal, out Collider hitCollider))
                continue;

            if (hitCollider == null)
                continue;

            Transform hitHost = ResolveShadowHostTransform(hitCollider);
            if (currentMovingShadowHost != null && hitHost == currentMovingShadowHost)
                return hitCollider;

            if (fallback == null)
                fallback = hitCollider;
        }

        return fallback;
    }

    bool TryGetShadowCasterAtPoint(Vector3 point, Vector3 normal, out Collider casterCollider)
    {
        casterCollider = null;
        ...
        for (int i = 0; i < gameplayLights.Length; i++)
        {
            Light light = gameplayLights[i];
            if (!TryBuildLightRay(point, normal, light, out Vector3 origin, out Vector3 dir, out float maxDistance))
                continue;

            if (!Physics.Raycast(origin, dir, out RaycastHit hit, maxDistance, occluderMask, QueryTriggerInteraction.Ignore))
                continue;

            if (hit.collider == null)
                continue;

            Transform hitHost = ResolveShadowHostTransform(hit.collider);
            if (currentMovingShadowHost != null && hitHost == currentMovingShadowHost)
            {
                casterCollider = hit.collider;
                return true;
            }
        ...
        return casterCollider != null;
    }

    bool TryBuildLightRay(Vector3 point, Vector3 normal, Light light, out Vector3 origin, out Vector3 dir, out float maxDistance)
    {
        origin = point + normal.normalized * lightProbeSurfaceOffset;
        dir = Vector3.zero;
        maxDistance = 0f;
        ...
        switch (light.type)
        {
            case LightType.Directional:
                {
                    dir = -light.transform.forward.normalized;
            ...
            case LightType.Point:
                {
                    ...
                    if (light.range > 0f && dist > light.range) return false;
            ...
            case LightType.Spot:
                {
                    Vector3 fromLightToPoint = origin - light.transform.position;
                    float dist = fromLightToPoint.magnitude;
                    if (dist <= 0.0001f) return false;
                    if (light.range > 0f && dist > light.range) return false;

                    Vector3 fromLightToPointDir = fromLightToPoint / dist;
                    float angle = Vector3.Angle(light.transform.forward, fromLightToPointDir);
                    if (angle > light.spotAngle * 0.5f) return false;

                    dir = -fromLightToPointDir;
                    maxDistance = dist;
                    break;
                }
        ...
        if (Vector3.Dot(normal.normalized, dir) <= 0.0001f)
            return false;

        return true;
    }

    Transform ResolveShadowHostTransform(Collider col)
    {
